...
namespace Interaction
{
    public class HoverPartialGrid : MonoBehaviour
    {
        ...
        private float faceTileSize = 1f;
        private float faceTileOffset = 0.001f;
        private float ghostScale = 1f;
        private float threshold = 0.222f;
        private float positionCheckTolerance = 0.001f;
        ...
        void Start()
        {
            faceTile = CreatePreview(PrimitiveType.Quad, Settings.Instance.placementGridPreviewColor, out faceTileR);
            ghostCube = CreatePreview(PrimitiveType.Cube, Settings.Instance.placementPreviewColor, out ghostCubeR);
        }
        ...
        void Update()
        {
            ...
            ghostCubeR.material.color = IsBlocked(sp) 
                ? Settings.Instance.placementPreviewColorBlocked 
                : Settings.Instance.placementPreviewColor;
        ...
        Vector3 GetSpawnPos(RaycastHit h, float m, int a, int b)
        {
            ...
            Vector3 f = Mathf.Abs(h.normal.x) > 0.5f ? new Vector3(0, a, b) :
                        Mathf.Abs(h.normal.y) > 0.5f ? new Vector3(a, 0, b) :
                        new Vector3(a, b, 0);
            return p + m * f;
        }
        ...
        void ShowFaceSubTile(RaycastHit h, int a, int b)
        {
            ...
            Vector2 localOffset = Mathf.Abs(h.normal.x) > 0.5f ?
                new Vector2(localHit.y - localFaceCenter.y, localHit.z - localFaceCenter.z) :
                Mathf.Abs(h.normal.y) > 0.5f ?
                new Vector2(localHit.x - localFaceCenter.x, localHit.z - localFaceCenter.z) :
                new Vector2(localHit.x - localFaceCenter.x, localHit.y - localFaceCenter.y);
            ...
            Vector3 finalWorld = h.collider.transform.TransformPoint(finalLocal) + h.normal * faceTileOffset;
            faceTileR.sharedMaterial.color = Settings.Instance.placementGridPreviewColor;
