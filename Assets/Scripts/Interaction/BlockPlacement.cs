...
public class BlockPlacements : MonoBehaviour
{
    ...
    private void CalculatePositionPlacement(Vector3 hitOnNormalPos, Vector3 hitNormal, RaycastHit hit, float multiplier)
    {
        ...
        {
            Debug.Log("A block already exists at " + spawnPos + ". Placement aborted.");
            return;
        }

        SpawnBlock(spawnPos, hit);
        Debug.Log($"Spawned block at {spawnPos} using hit normal {hitNormal} with multiplier {multiplier}");
    }
